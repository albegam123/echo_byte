...
impl BLEDevice {
    ...
    pub fn deinit_full() -> Result<(), EspError> {
        // `BLEAdvertising::reset()` calls `ble_gap_adv_active()`, which needs
        // the NimBLE host state to still exist. Upstream 0.12.0 performed this
        // after `nimble_port_deinit()`, causing a LoadProhibited exception on
        // ESP-IDF 5.5. Reset the live GAP state first, stop/deinit the host,
        // then release the Rust-side GATT services and callbacks.
        if INITIALIZED.load(Ordering::Acquire) {
            #[cfg(not(esp_idf_bt_nimble_ext_adv))]
